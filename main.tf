...
resource "spacelift_stack" "main" {
  name        = "metagpt-custom-agent-spacelift-${formatdate("YYYYMMDDhhmmss", timestamp())}"
  repository  = "metagpt-custom-agent"
  branch      = "main"
...
resource "spacelift_context" "main" {
  name        = "metagpt-custom-agent-context-spacelift-${formatdate("YYYYMMDDhhmmss", timestamp())}"
  description = "Shared configuration for React application (Spacelift managed)"
}

resource "spacelift_policy" "main" {
  name        = "metagpt-custom-agent-policy-spacelift-${formatdate("YYYYMMDDhhmmss", timestamp())}"
  description = "Shared configuration for React application (Spacelift managed)"
